...
-- 데이터 추가 tasks (이름) values (값) *수가 맞지 않으면 작동 안 됨
INSERT INTO tasks (_id, title, description, date, isCompleted, isImportant, userId) VALUES ('1234', '할일1', '할일1 설명', '2025-08-01', false, false, 'seunghee');


-- 데이터 조회
SELECT * FROM tasks WHERE userId = 'seunghee' ORDER BY created_at DESC(ASC);


-- 특정 사용자 데이터 필터 조회
SELECT * FROM tasks WHERE userId = 'seungheebsh'
